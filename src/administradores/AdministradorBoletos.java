...
public class AdministradorBoletos {
	...
	private void cancelarBoletoBD() throws SQLException {
		 ...
		 * "Esta seguro de que desea cancelar la funcion?", "Actualizar Funcion",
		 * JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE); if (reply ==
		 * JOptionPane.YES_OPTION) { cancelar.confirmar();
		 * JOptionPane.showMessageDialog(null, "Ha cancelado la Funcion", "",
	...
	private Double calcularPrecioBoleto() throws SQLException {
		...
		if (coordenada[0].equals("H") || coordenada[0].equals("G")) {
			if (coordenada[1].equals("7") || coordenada[1].equals("6") || coordenada[1].equals("9")
					|| coordenada[1].equals("10") || coordenada[1].equals("11") || coordenada[1].equals("12")
					|| coordenada[1].equals("13") || coordenada[1].equals("14")) {
		...
		return precioObra;// cambiar el return por precioObra cuando este lista la parte grafica de
							// asientos
	}

	private Double obtenerPrecioObra() throws SQLException {
		Double consulta = new DAO().buscarPrecioObraJoinFunciones("precioObra", DAO.FUNCION, DAO.OBRA, "idObra",
				obtenerIDObra());
		return consulta;
	}
	...
	private void imprimirBoleto() throws SQLException {
		...
		Object[][] buscar = new DAO().buscarObrasJoinFunciones(
				"b.idBoleto, b.coordenadaAsiento, b.idFuncion, f.idObra, o.nombreObra, o.descripcionObra, o.duracionObra, f.fechaFuncion, f.inicioFuncion",
				DAO.FUNCION, DAO.BOLETO, "b.idfuncion", obtenerIDFuncion(), obtenerIDObra());// Consulta super culera
		...
		/*
		 * Adjunto el query: select b.idBoleto, b.coordenadaAsiento, b.idFuncion,
		 * f.idObra, o.nombreObra, o.descripcionObra, o.duracionObra, f.fechaFuncion,
		 * f.inicioFuncion from funcion f join boleto b on f.idfuncion = b.idFuncion,
		 * obra o where b.idfuncion = 25 and f.idObra = 1
		 * 
		 * los valores 1 y 25 son ejemplos a ver si jala, no pude revisar ya que mi
		 * tabla de boletos no tenia valores, inventele y pruebe
		 * 
		 */
